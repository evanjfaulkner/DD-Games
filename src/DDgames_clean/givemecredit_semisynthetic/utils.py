...
def fit_logistic_regression(X, Y, l2_penalty, tol=1e-7, theta_init=None):
    ...
    while gap > tol:
        ...
        c = exp_tx / (1 + exp_tx) - Y
        gradient = 1.0 / n * np.sum(X * c[:, np.newaxis], axis=0) + l2_penalty * np.append(theta[:-1], 0)
...
def fit_pp_logistic_regression(X, Y, mu, l2_penalty, tol=1e-7, theta_init=None):
    """Fit a logistic regression model via gradient descent.
    ...
    while gap > tol:
        ...
        # take theta gradients
        exp_tx = np.exp(X @ theta)
        c = exp_tx / (1 + exp_tx) - Y
        gradient_theta = 1.0 / n * np.sum(X * c[:, np.newaxis], axis=0) + l2_penalty * np.append(theta[:-1], 0)

        # take x gradients
        exp_tx = np.exp(X @ theta)
        c = exp_tx / (1 + exp_tx) - Y
        gradient_x = 1.0 / n * mu.T@np.sum(theta * c[:, np.newaxis], axis=0) 
        
        new_theta = theta - eta * (gradient_theta+gradient_x)
