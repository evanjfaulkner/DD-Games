...
def make_confusion_matrix(cf,
                          ...
                          figsize=None,
                          cmap='Blues',
                          title=None, fontsize=24, save=False, filename=None, verbose=True, ax=None):
    '''
    ...
    if ax==None:
        ...
        aa=sns.heatmap(cf,annot=box_labels,fmt="",cmap=cmap,cbar=False,xticklabels=categories,yticklabels=categories, annot_kws={"size": fontsize}, ax=ax)
        aa.axes.get_yaxis().set_visible(False)
        ax.tick_params(labelsize=fontsize)
        if xyplotlabels:
            ax.set_ylabel('True label',fontsize=fontsize)
            ax.set_xlabel('Predicted label' + stats_text,fontsize=fontsize)
        else:
